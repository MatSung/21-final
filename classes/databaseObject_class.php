...
class databaseObject extends DatabaseConnection
{
    ...
    public function __construct($table1, $settings = [])
    {
        ...
        if ($table1 == "klientai") {
            //subcategories
            array_push($this->subcategoryList, $this->selectIDValuePairAction("klientai_teises", "pavadinimas"));
            array_push($this->subcategoryList, $this->selectIDValuePairAction("imones", "pavadinimas"));
        ...
        if ($table1 == "imones") {
            //subcategories
            array_push($this->subcategoryList, $this->selectIDValuePairAction("imones_tipas", "pavadinimas"));
            $tables = ["imones_tipas"];
        ...
        if ($table1 == "vartotojai") {
            //subcategories
            array_push($this->subcategoryList, $this->selectIDValuePairAction("vartotojai_teises", "pavadinimas"));
    ...
    public function insertEntry()
    {
        if ($this->type == "imones_tipas" || $this->type == "vartotojai_teises") {
            ...
            return $this->insertAction($this->type, ["vardas", "pavarde", "teises_id", "aprasymas", "imones_id", "pridejimo_data"], ["'" . $_POST["vardas"] . "'", "'" . $_POST["pavarde"] . "'", $_POST["teises"], "'" . $_POST["aprasymas"] . "'", $_POST["imone"], "'" . date("Y-m-d") . "'"]);
            //return 1;
            ...
            $this->insertAction($this->type, ["pavadinimas", "tipas_id", "aprasymas"], ["'" . $_POST["pavadinimas"] . "'", "'" . $_POST["imones_tipas"] . "'", "'" . $_POST["aprasymas"] . "'"]);
        }
        //return 0;
    ...
    /**
     * 
     * Function to get a full table
     *
     * @param bool  $editRights will add editing capability
     * @return array will return array of keys
     */
    public function drawTable($editRights = 0)
    {
        ...
        if ($editRights) {
            ...
            foreach ($keys as $key) {
                echo "<td>";
                //if special key then do whatever that special key needs
                if ($key == "id") {
                    echo "<button class='btn btn-primary' name='" . $this->type . "InsertEntry' type='submit' class=class='mb-2 mr-2 btn btn-primary'>+</button>";
                } else if ($key == "pridejimo_data" || $key == "registracijos_data") {
                    echo "<input disabled type='text' class='form-control-sm form-control' name='" . $key . "' value='" . date("Y-m-d") . "'>";
                } else if ($key == "imones_tipas" || $key == "teises") {
                    echo "<select class='form-control-sm form-control' name='" . $key . "'>";
                    foreach ($this->subcategoryList[0] as $subcat) {
                        echo "<option value='" . $subcat["id"] . "'>" . $subcat["pavadinimas"] . "</option>";
                    }
                    echo "</select>";
                } else if ($key == "imone") {
                    echo "<select class='form-control-sm form-control' name='" . $key . "'>";
                    foreach ($this->subcategoryList[1] as $subcat) {
                        echo "<option value='" . $subcat["id"] . "'>" . $subcat["pavadinimas"] . "</option>";
                    }
                    echo "</select>";
                } else if ($key == "paskutinis_prisijungimas") {
                    echo "<input disabled type='text' class='form-control-sm form-control' name='" . $key . "' value='0000-00-00'>";
                } else {
